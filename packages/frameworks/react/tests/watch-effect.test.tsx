...
describe("react: watchEffect basics", () => {
...
describe("react: effect declaration edges", () => {
...
describe("react: dep value semantics", () => {
...
describe("react: scheduling and races", () => {
  ...
  // SKIPPED — not applicable: the reconciler runs after every render by design here
  test.skip("a restart does not subscribe the reconciler to what setup() reads", async () => {
...
describe("react: restart coalescing", () => {
...
describe("react: controlled open prop (the dialog shape)", () => {
...
describe("react: issue #3302 — dialog open-state props toggled live", () => {
  let focusCalls = 0

  function dialogMachine() {
    return createMachine<any>({
      props: ({ props }) => ({ modal: true, trapFocus: true, preventScroll: true, ...props }),
      initialState: () => "open",
      states: {
        open: {
          entry: ["setInitialFocus"],
          effects: ["trapFocus", "preventScroll", "hideContentBelow"],
          on: { CLOSE: { target: "closed" } },
        },
        closed: {},
      },
      implementations: {
        actions: { setInitialFocus: () => void focusCalls++ },
        effects: {
          trapFocus: ({ prop, watchEffect }: any) =>
            watchEffect([() => prop("trapFocus")], () => {
              if (!prop("trapFocus")) return
              document.body.setAttribute("data-trapped", "")
              return () => document.body.removeAttribute("data-trapped")
            }),
          preventScroll: ({ prop, watchEffect }: any) =>
            watchEffect([() => prop("preventScroll")], () => {
              if (!prop("preventScroll")) return
              document.body.style.overflow = "hidden"
              return () => document.body.style.removeProperty("overflow")
            }),
          hideContentBelow: ({ prop, watchEffect }: any) =>
            watchEffect([() => prop("modal")], () => {
  ...
  beforeEach(() => {
    focusCalls = 0
    document.body.removeAttribute("data-trapped")
    document.body.removeAttribute("aria-hidden")
    document.body.style.removeProperty("overflow")
  })

  test("maximized → minimized → maximized releases and reclaims focus/scroll, dialog stays open", async () => {
    const view = await renderWithProps(dialogMachine(), { modal: false, trapFocus: true, preventScroll: true })

    expect(document.body.hasAttribute("data-trapped")).toBe(true)
    expect(document.body.style.overflow).toBe("hidden")
    expect(document.body.hasAttribute("aria-hidden")).toBe(false)
    expect(focusCalls).toBe(1)

    await view.setProps({ trapFocus: false, preventScroll: false })

    expect(document.body.hasAttribute("data-trapped")).toBe(false)
    expect(document.body.style.overflow).toBe("")
    expect(view.service.state.get()).toBe("open")
    expect(focusCalls).toBe(1) // entry action did not re-run, so focus was not stolen back

    await view.setProps({ trapFocus: true, preventScroll: true })
  ...
  test("modal can be toggled independently of trapFocus", async () => {
    const view = await renderWithProps(dialogMachine(), { modal: true, trapFocus: true, preventScroll: true })
    expect(document.body.getAttribute("aria-hidden")).toBe("true")

    await view.setProps({ modal: false })
    expect(document.body.hasAttribute("aria-hidden")).toBe(false)
    expect(document.body.hasAttribute("data-trapped")).toBe(true)
  })

  test("closing tears down whatever is still active", async () => {
    const view = await renderWithProps(dialogMachine(), { modal: true, trapFocus: true, preventScroll: true })
...
describe("react: teardown reason", () => {
