import { countDecimals, toFixed } from "./number-utils"
...
export interface RangeUtilOptions {
  min?: number
  max?: number
  precision?: number
  step?: number
...
export class RangeUtil {
  ...
  constructor(public options: RangeUtilOptions) {
  ...
  static transform(inputTuple: RangeTuple, outputTuple: RangeTuple) {
    const input = RangeUtil.fromTuple(inputTuple)
    const output = RangeUtil.fromTuple(outputTuple)
  ...
  static valuesToRanges(options: ValuesToRangesOptions) {
    return options.value
      ...
      .map(([min, max, value]) => new RangeUtil({ min, max, value }))
  }
   ...
   */
  withOptions(options: Partial<RangeUtilOptions>) {
    return new RangeUtil({ ...this.options, ...options })
  }
   ...
   */
  valueOf() {
    return RangeUtil.parse(this.value)
  }
   ...
   * value passed.
   */
  fromPercent(percent: number | RangeUtil) {
    percent = percent instanceof RangeUtil ? percent.valueOf() : percent
  ...
  increment(step = this.step) {
    if (this.value === "") {
      this.value = RangeUtil.parse(step)
    } else {
   ...
   */
  snapToStep(value: number | RangeUtil) {
    const current = this.valueOf()
    value = value instanceof RangeUtil ? value.valueOf() : value
