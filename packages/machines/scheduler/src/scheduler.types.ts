...
/** One lane down the timeline — a resource, or a single unnamed lane. */
...
export interface SlotSelectDetails {
  ...
  start: CalendarDateTime
  /** End of the selected slot range. */
  end: CalendarDateTime
  /** Whether the selection originated in an all-day context (month cell or the all-day row). */
  allDay: boolean
  /** Which gesture produced the selection — lets consumers branch UX (e.g. drag opens a popover, double-click opens a full dialog). */
  action: "click" | "drag"
  /** The resource whose column the gesture happened in. */
...
export interface DragState<T extends SchedulerPayload = SchedulerPayload> {
  ...
  /**
   * The event's start/end at the moment the gesture began — restore target for escape-to-cancel, and anchor for the "origin" outline overlay.
   */
  origin: { start: CalendarDateTime; end: CalendarDateTime }
  /** Whether the gesture currently targets the all-day row. */
  allDay: boolean
}
...
export interface SchedulerProps<T extends SchedulerPayload = SchedulerPayload>
  ...
  /** Resources (people, rooms, equipment) to schedule against. */
  ...
  /** Fires when a user selects an empty slot — either by single click or by drag-release. */
  ...
  /**
   * Fires on double-click of an empty slot — the conventional "create event" fast-path that bypasses slot selection (selectedSlot is not set).
   */
  ...
  /** Return false to prevent dragging an event. */
  canDragEvent?: ((event: SchedulerEvent<T>) => boolean) | undefined
  /** Return false to prevent resizing an event. */
  canResizeEvent?: ((event: SchedulerEvent<T>) => boolean) | undefined
  /** Decides whether a drag or resize may commit at its current position. */
  ...
  /** IANA time zone driving `today` and every formatter. */
  timeZone?: string | undefined
  /** Override the first day of the week (0=Sun…6=Sat). */
...
export interface DayColumnState {
  ...
  /** Whether the drag/resize gesture started in this column — i.e. the origin outline renders here. */
...
export interface DayCellTriggerProps {
  date: CalendarDateTime
  /** Reference date for the containing month grid. */
  referenceDate?: CalendarDateTime
}

export interface MonthGridProps {
  /** Reference date whose month the grid represents. */
  date: CalendarDateTime
}
...
export interface DayCellProps {
  date: CalendarDateTime
  /**
   * Reference date for month-based layouts — used to decide whether the cell is "outside" the current month (greys it out).
   */
  referenceDate?: CalendarDateTime
  /**
   * Marks the cell as part of the all-day row.
   * @default false
   */
  allDay?: boolean
}
...
export interface EventProps<T extends SchedulerPayload = SchedulerPayload> {
  ...
  /** The bar to place, from `getAllDaySegments()`. */
...
export interface HourRange {
  ...
  /** Hours from start..end inclusive, each with its localized label and grid position pre-computed. */
  hours: HourEntry[]
}
...
export interface SchedulerApi<T extends PropTypes = PropTypes, P extends SchedulerPayload = SchedulerPayload> {
  ...
  /** The columns to render — one per visible day, or one per day × resource when grouping by resource. */
  ...
  /** Visible events grouped by day and sorted by start. */
  ...
  /** Whether the user is currently resizing an event. */
  isResizing: boolean
  /** State of the active drag or resize — null when idle. */
  dragState: DragState<P> | null
  /** Slot the user selected (clicked or drag-selected). */
  selectedSlot: { start: CalendarDateTime; end: CalendarDateTime } | null
  /** Day cell that currently has keyboard focus in a month/year grid. */
  focusedDate: CalendarDateTime | null
  /** Imperatively move keyboard focus to a specific date. */
  setFocusedDate: (date: CalendarDateTime) => void
  /** Drop keyboard focus from the month grid. */
  clearFocusedDate: () => void
  /** Switch to another view. */
  setView: (view: ViewType) => void
  /** Move the visible range to contain this date. */
  setDate: (date: CalendarDateTime) => void
  /** Jump the visible range to today. */
  goToToday: () => void
  /** Advance by one view-sized step. */
  goToNext: () => void
  /** Go back by one view-sized step. */
  goToPrev: () => void
  /** Dismiss the selected slot highlight (e.g. when a create dialog closes). */
  clearSelectedSlot: () => void
  /** Interaction state for an event — dragging, focused, conflicting, and so on. */
  getEventState: (id: string) => EventStateDetail
  /**
   * Day-level flags for the given column — use to conditionally render custom UI (drop-target hint, day-specific styling) without reconstructing the comparisons from `dragState` / `selectedSlot`.
   */
  ...
  /** Events touching a day, multi-day ones included. */
  ...
  /** Events overlapping a time slot. */
  ...
  /** The DOM element rendered by `getSelectedSlotProps` for the active `selectedSlot` day. */
