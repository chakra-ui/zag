...
export interface IntlTranslations {
  placeholder: (locale: string) => { year: string; month: string; day: string; hour: string; min: string }
}
...
interface PublicContext extends DirectionProperty, CommonProperties {
  ...
  /**
   * Whether the datetimepicker is open
   */
  open?: boolean
  /**
   * Whether the datetimepicker open state is controlled by the user
   */
...
export interface MachineApi<T extends PropTypes = PropTypes> {
   ...
   */
  setValue(values: CalendarDateTime[]): void
  /**
   * Sets the focused date to the given date.
   */
  setFocusedValue(value: CalendarDateTime): void
  /**
