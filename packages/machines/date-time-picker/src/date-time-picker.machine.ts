...
import { dom } from "./date-time-picker.dom"
import type { DateValue, DateView, MachineContext, MachineState, UserDefinedContext } from "./date-time-picker.types"
import { adjustStartAndEndDate, sortDates } from "./date-time-picker.utils"
  ...
  return createMachine<MachineContext, MachineState>(
    {
      ...
      computed: {
        ...
        formattedValue(ctx) {
          ...
          return ctx.value.map((date) =>
            (ctx.format?.(date) ?? formatter.format(date.toDate(ctx.timeZone))).replace(/, /g, " "),
          )
        },
      },
      ...
      on: {
        ...
        "INPUT.CHANGE": {
          actions: ["focusParsedDateTime"],
        },
        "INPUT.ENTER": {
          actions: ["focusParsedDateTime", "selectFocusedDate"],
        },
        "INPUT.FOCUS": {
    ...
    },
    {
      ...
      actions: {
        ...
        focusParsedDate(ctx, evt) {
          ...
          const date = parseDateTimeString(evt.value, ctx.locale, ctx.timeZone)
          if (!date) return

          set.focusedValue(ctx, date)
        },
        focusParsedDateTime(ctx, evt) {
          if (evt.index == null) return
      ...
      compareFns: {
        startValue: isDateTimeEqual,
        endValue: isDateTimeEqual,
        focusedValue: isDateTimeEqual,
        value: isDateTimeEqualFn,
      },
    },
  )
}
...
const isDateTimeEqualFn = (a: DateValue[], b: DateValue[]) => {
  if (a.length !== b.length) return false
  return a.every((date, index) => isDateTimeEqual(date, b[index]))
}

const set = {
  value(ctx: MachineContext, value: DateValue[]) {
    if (isDateTimeEqualFn(ctx.value, value)) return
    ctx.value = value
    invoke.change(ctx)
  },

  focusedValue(ctx: MachineContext, value: DateValue | undefined) {
    if (!value || isDateTimeEqual(ctx.focusedValue, value)) return
