...
export function proxy<T extends object>(initialObject: T = {} as T): T {
  ...
  const createSnapshot = (target: T, receiver: any) => {
    ...
    const snapshot: any = Array.isArray(target) ? [] : Object.create(Object.getPrototypeOf(target))
...
export function subscribe<T extends object>(proxyObject: T, callback: (ops: Op[]) => void, notifyInSync?: boolean) {
