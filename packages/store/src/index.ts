  ...
  proxyFunction = <T extends object>(initialObject: T): T => {
    ...
    const handler: ProxyHandler<T> = {
      get(target: T, prop: string | symbol, receiver: object) {
        if (prop === PROXY_STATE) {
          const state: ProxyState = [target, receiver, version, createSnapshot, listeners]
          return state
        }
      ...
      deleteProperty(target: T, prop: string | symbol) {
        ...
        const childListeners = ((prevValue as any)?.[PROXY_STATE] as ProxyState | undefined)?.[4]
        if (childListeners) {
...
export function snapshot<T extends object>(proxyObject: T, handlePromise?: HandlePromise): Snapshot<T> {
  ...
  const [target, receiver, version, createSnapshot] = (proxyObject as any)[PROXY_STATE] as ProxyState
