            ...
            {documents.map((doc) => (
              <tr key={doc.id} style={{ borderBottom: "1px solid #e5e7eb" }}>
                ...
                <td style={{ padding: "12px" }}>
                  <button
                    {...mergeProps(api.getTriggerProps({ value: `${doc.id}` }), {
      ...
      <Portal>
        <div {...api.getPositionerProps()}>
          <div {...api.getContentProps()}>
            <div {...api.getItemProps({ value: "rename" })}>Rename (value: {api.activeTriggerValue})</div>
            <div {...api.getItemProps({ value: "delete" })}>Delete (value: {api.activeTriggerValue})</div>
          </div>
        </div>
      </Portal>
    </main>
