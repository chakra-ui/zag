...
export default function Page() {
  const [selectedValue, setSelectedValue] = useState<datePicker.DateValue[]>([])

  // Calculate max date based on the first selected date (start date)
  const maxDate = useMemo(() => {
    const [start, end] = selectedValue.sort((a, b) => a.compare(b))
    if (start && !end) return start.add({ days: 3 })
  }, [selectedValue])
      ...
      <div {...api.getRootProps()}>
        <div {...api.getContentProps()} className={styles.Content}>
          ...
          <div hidden={api.view !== "day"}>
            <div {...api.getViewControlProps({ view: "day" })} className={styles.ViewControl}>
