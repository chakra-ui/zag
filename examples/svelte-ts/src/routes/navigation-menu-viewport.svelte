      ...
      <div {...api.getViewportPositionerProps()}>
        <Presence {...api.getViewportProps()}>
          ...
          >
            ...
            {#each renderLinks({ value: "products", items: ["Case Studies", "Success Stories", "Integration Partners", "Security & Compliance"] }) as { item, index, value }}
              <a href="#" {...api.getLinkProps({ value })} key={`${value}-${item}-${index}`}>
                {item}
              </a>
            {/each}
          ...
          <Presence
            ...
            style="grid-template-columns: 1fr 1fr; width: 450px"
          >
            ...
            {#each renderLinks({ value: "company", items: ["Investors", "Partners", "Corporate Responsibility"] }) as { item, index, value }}
              <a href="#" {...api.getLinkProps({ value })} key={`${value}-${item}-${index}`}>
                {item}
              </a>
            {/each}
          ...
          >
            {#each renderLinks({ value: "developers", items: ["API Documentation", "SDKs & Libraries", "Developer Guides", "Code Samples", "Webhooks", "GraphQL Explorer"] }) as { item, index, value }}
              <a href="#" {...api.getLinkProps({ value })} key={`${value}-${item}-${index}`}>
                {item}
              </a>
            {/each}
            
            {#each renderLinks({ value: "developers", items: ["Developer Community", "Changelog", "Status Page", "Rate Limits"] }) as { item, index, value }}
              <a href="#" {...api.getLinkProps({ value })} key={`${value}-${item}-${index}`}>
                {item}
              </a>
            {/each}
