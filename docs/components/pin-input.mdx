...
- Automatically focuses the next field on typing and focuses previous field on deletion
...
The pin-input's connect function exposes `containerProps` and `getInputProps` you can use to compose the UI.
...
To trigger smartphone OTC auto-suggestion, it's usually recommended to apply `autocomplete="one-time-code"` to the input
field. The pin-input machine provides support for this. To enable this, set the `otp` context property to `true`.
...
into it, PinInput attempts to validate the string and fill the other inputs.
