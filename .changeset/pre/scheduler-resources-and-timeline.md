...
- Add drag and drop constraints via `canDropEvent`, and external drag-and-drop via `dataTransferFormat` and
  `onEventReceive`. Items can be dropped on the all-day row as well as a timed slot, and `onEventReceive` reports which
  region took the drop.
...
- Fixed issue where all-day bars past the first level spilled out of the row into the time grid. The row now reports its
  level count so it can size itself, and `maxAllDayRows` caps it, collapsing the rest into per-day counts read from
  `getAllDayOverflow()`.
