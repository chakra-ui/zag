...
The avatar machine provides a `onLoad` and `onError` event that you can listen
to. This is useful when you want to perform an action when the image loads or
fails to load.
...
const [state, send] = useMachine(
  avatar.machine({
    ...
    onError() {
      // Do something when the image fails to load
    },
  }),
)
```
