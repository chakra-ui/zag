...
### Resuming autoplay when the page becomes visible

Autoplay pauses when the document is hidden. Set `resumeOnVisibilityChange` to
`true` to restart it when the document becomes visible again. This only resumes
autoplay that was paused because the document was hidden; it does not override a
manual pause.
