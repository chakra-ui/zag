...
- Priority-based queue system for ordering queued toasts
...
- `priority` — Queue priority (1–8, lower is higher). Auto-assigned based on
  `type` and `action` when omitted.
...
When the max number of visible toasts is reached, queued toasts are sorted by
priority before being displayed. Priority ranges from `1` (highest) to `8`
(lowest).

Default priorities are assigned automatically based on toast type and whether it
has an action:
...
You can override the priority by passing the `priority` option:

```tsx
toaster.create({
  title: "Critical update",
  type: "info",
  priority: 1, // treat as highest priority
})
```
...
[data-toast-root][data-state="closed"] {
  transition:
    translate 400ms,
    scale 400ms,
    opacity 200ms;
...
```css
[data-toast-root][data-type="info"] {
  /* Styles for the specific toast type */
}

[data-toast-root][data-type="error"] {
  /* Styles for the error toast type */
}

[data-toast-root][data-type="success"] {
  /* Styles for the success toast type */
}

[data-toast-root][data-type="loading"] {
