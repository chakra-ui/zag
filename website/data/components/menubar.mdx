...
A menubar groups several menus — like the File / Edit / View bar in a desktop
app — into a single widget. Each menu is a regular Zag.js menu; the menubar
...
- Once a menu is open, hovering or arrowing to a sibling switches to it without
  a click.
...
The menubar gives each menu a small config object through
`api.getMenuContext()`. Pass it to the menu's `menubar` prop — that's what makes
the menu behave as part of the bar instead of a standalone dropdown. A context
is the easiest way to thread it down:
...
The config carries the menubar's `rootId`, `activeId`, `disabled`, and
`orientation`, so a menu always reflects the bar it belongs to.
...
Set `orientation` to `"vertical"` to stack the triggers. Use `positioning` so
the menus open to the side instead of below.
...
```css
[data-menubar-root][data-orientation="horizontal|vertical"] {
  /* styles per orientation */
}

[data-menubar-root][data-disabled] {
  /* styles when the whole bar is disabled */
}

/* present while any menu in the bar is open */
[data-menubar-root][data-has-open-menu="true"] {
  /* ... */
}
```

Triggers are owned by the menu, so style them with the menu's `trigger` part.
See the [menu styling guide](/components/menu#styling-guide).
