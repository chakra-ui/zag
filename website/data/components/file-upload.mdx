...
- **Input**: The underlying input element that handles the file upload.
...
When files are uploaded, the `onChange` callback is invoked with the details of
...
context, and ensure you render the input element `api.inputProps`
...
To disable the drag and drop functionalty, set the `dropzone` context property
to `false`.
