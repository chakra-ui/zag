...
- Support for RGBA, HSLA, HEX, HSBA, OKLAB and OKLCH formats
...
format to `hsla`, `hsba`, `oklab` or `oklch` by using the `format` context property.
...
#### Swatch thumb in relation to direction state

As swatch's thumb position is dependent on direction, you might want to style it differently based on `dir` attribute of the swatch
