...
When an item is highlighted with the pointer or keyboard, use the `onHighlight`
...
const [state, send] = useMachine(
  select.machine({
    id: useId(),
    onHighlight(details) {
      // details => { value: string | null, item: Item | null }
      console.log(details)
    },
  }),
)
...
When an item is selected, use the `onChange` property to listen for the change
