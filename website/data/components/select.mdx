...
- `content` — the dismissable popup wrapper (presentational by default, or
  `role="dialog"` when `composite: false`).
...
Use `composite: false` when content is a complex composition (search input, tabs)
to make `content` carry `role="dialog"` for assistive technology.
