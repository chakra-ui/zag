...
### Running code after enter animation

> `onEnterComplete` is only available from `v1.43.2`

Use `onEnterComplete` to run logic after the enter animation finishes. If the
element has no enter animation, the callback is invoked on the next frame after
it mounts.

> The callback doesn't run on the initial render, only when `present` changes to
> `true`.
