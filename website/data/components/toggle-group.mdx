...
> Each part includes a component-specific data attribute to help identify it in the DOM.
...
When pressed, `data-pressed` is applied to the toggle button.
...
Use native focus selectors to style the focused toggle button. Use
`:focus-visible` for keyboard focus rings and `:focus-within` if the root should
be styled while a toggle button is focused.

```css
[data-toggle-group-root]:focus-within {
  /* styles for the focused root */
}

[data-toggle-group-item]:focus-visible {
  /* styles for the focused toggle */
}
```
...
```css
[data-toggle-group-root][data-disabled] {
  /* styles for the root */
}
...
Uses roving tabindex to manage focus movement among toggle buttons. Items are
rendered as buttons with `aria-pressed` in both single and multiple selection
modes.
