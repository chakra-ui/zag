...
const service = useMachine(numberInput.machine, {
  onValueChange(details) {
    // details => { value: string, valueAsNumber: number, reason?: ValueChangeReason }
    console.log("value is:", details.value)
  },
})
```

`details.reason` says what caused the change, so you can tell a deliberate entry
from a drag.

| Reason            | Meaning                                      |
| ----------------- | -------------------------------------------- |
| `input-change`    | The user typed in the input                  |
| `input-blur`      | The value was clamped or formatted on blur   |
| `keyboard`        | Arrow, page, home or end key                 |
| `increment-press` | The increment trigger was pressed            |
| `decrement-press` | The decrement trigger was pressed            |
| `wheel`           | The mouse wheel was scrolled over the input  |
| `scrub`           | The scrubber was dragged                     |
| `script`          | Changed programmatically, such as `setValue` |

### Listening for value commit

`onValueChange` fires for every step of a drag. Use `onValueCommit` to react
only when the value settles, which is the better place for saving or validating.

It fires when the input is blurred or `Enter` is pressed, when a stepper press
or scrub gesture ends, and alongside each key or wheel step. A scrub that
streams a hundred steps commits once.

```tsx
const service = useMachine(numberInput.machine, {
  onValueCommit(details) {
    // details => { value: string, valueAsNumber: number, reason?: ValueChangeReason }
    console.log("committed:", details.value, "via", details.reason)
  },
})
```
...
```css
[data-number-input-root][data-disabled] {
  /* disabled styles for the input */
}

[data-number-input-input][data-disabled] {
  /* disabled styles for the input */
}

[data-number-input-label][data-disabled] {
  /* disabled styles for the label */
}

[data-number-input-increment-trigger][data-disabled] {
  /* disabled styles for the increment button */
}

[data-number-input-decrement-trigger][data-disabled] {
...
```css
[data-number-input-root][data-invalid] {
  /* disabled styles for the input */
}

[data-number-input-input][data-invalid] {
  /* invalid styles for the input */
}

[data-number-input-label][data-invalid] {
...
```css
[data-number-input-input][data-readonly] {
...
The spin buttons can be styled individually with their respective
component-scoped data attributes.
...
[data-number-input-decrement-trigger] {
