...
Compose menu with [`async-list`](/utilities/async-list) and
`@zag-js/infinite-scroll` to load pages on open and fetch the next page as the
user scrolls.

- Keep `autoReload: false` so nothing fetches until the menu opens.
- Call `listApi.reload()` once from `onOpenChange` on the first open.
- Render a `LoadMore` child (owning `infinite-scroll.machine`) only while the
  menu is open, and keep its sentinel out of `getItemProps` so keyboard
  navigation skips it.

Install the extra packages alongside the menu package:

```bash
npm install @zag-js/async-list @zag-js/infinite-scroll
```

<Showcase id="MenuAsync" />

<CodeSnippet id="menu/async.mdx" />
...
```css
[data-menu-content][data-state="open|closed"] {
  /* styles for open or closed state */
}

[data-menu-trigger][data-state="open|closed"] {
  /* styles for open or closed state */
}
```
...
[data-menu-item][data-type="radio|checkbox"][data-highlighted] {
  /* styles for highlighted state */
}
```
...
[data-menu-item][data-type="radio|checkbox"][data-disabled] {
  /* styles for disabled state */
}
```
...
```css
[data-menu-item][data-type="radio|checkbox"][data-state="checked"] {
