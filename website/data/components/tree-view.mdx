...
When using checkbox selection (`onCheckedChange`), node and node checkbox
elements expose `data-checked` and `data-indeterminate` attributes for styling.

```css
[data-tree-view-node][data-checked] {
  /* styles for fully checked items */
}

[data-tree-view-node][data-indeterminate] {
  /* styles for partially checked items (some children selected) */
}

[data-tree-view-node-checkbox][data-checked] {
  /* styles for fully checked branches */
}

[data-tree-view-node-checkbox][data-indeterminate] {
  /* styles for partially checked branches */
}
```
