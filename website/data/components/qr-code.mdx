...
automatically centered within the QR code. Downloads and `getDataUrl` include
the overlay when it is rendered.
...
To export the QR code as an image, use `api.getDataUrl`. If an overlay is
rendered, it is included in the file.
...
> Overlay images must be same-origin or served with CORS headers. If the image
> cannot be read, the export falls back to the QR pattern.

You can also use `api.getDownloadTriggerProps` to create a download button.

<CodeSnippet id="qr-code/download.mdx" />
