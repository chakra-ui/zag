...
- Programmatic helpers like `api.resize`, `api.setCrop`, and
  `api.getCroppedImage`
...
Use `api.setCrop(rect)` to place the crop area at a rect in viewport
coordinates, like `api.crop`. Use it for presets, numeric fields, or a "center"
button.
...
The rect is constrained the same way as `initialCrop`: `minSize`/`maxSize`,
`aspectRatio` (or `cropShape: "circle"`), and the viewport bounds. When the rect
doesn't match the aspect ratio, the crop shrinks to fit inside it. Unlike
`api.reset()`, zoom, rotation, flip and pan are kept.

> `setCrop` does nothing when `fixedCropArea` is set. To set the crop on mount,
> use `initialCrop`.
...
Exports use the crop's natural resolution by default. Large images can exceed
browser canvas limits, so use `maxSize` to constrain output dimensions when
needed. Exports are never upscaled beyond natural resolution. The method
resolves to `null` if the browser cannot create or encode the canvas.
...
These are the absolute pixel bounds in the original image after inverting pan,
zoom, flip, and rotation:
...
//   corners: {
//     topLeft: { x: 250, y: 150 },
//     topRight: { x: 1250, y: 150 },
//     bottomRight: { x: 1250, y: 900 },
//     bottomLeft: { x: 250, y: 900 },
//   },
...
- Accounts for the rendered image size and inverts zoom, pan, flip, and rotation
- Under rotation, `x/y/width/height` is an axis-aligned bounding box of the
  oriented crop
- `corners` preserves the exact source quad as fractional coordinates
- `outputSize` is the crop size at natural resolution before export limits
...
// Backend:
// - Without rotation, crop with x/y/width/height, then apply flipX/flipY.
// - With rotation, map corners to outputSize with an affine transform.
```
...
api.getCropData()
// { x: 1000, y: 700, width: 500, height: 375, ... }
// Effective scale: (3000 / 600) / 2 = 2.5x
```
