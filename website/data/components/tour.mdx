---
title: Tour
description: Headless product tour and onboarding component for React, Vue, Solid, and Svelte. Step-by-step overlays with tooltip, dialog, and floating steps.
package: "@zag-js/tour"
---
...
The `action` property accepts one of the following built-in types, or a custom
function:

- `next`: Move to the next step.
- `prev`: Move to the previous step.
- `dismiss`: End the tour. Use this when the user is unlikely to want to see
  the tour again.
- `skip`: End the tour, similar to `dismiss`, but semantically intended for
  cases where the user might want to resume or restart the tour later.
- A custom function `(actionMap) => void`, which receives the same
  `next`/`prev`/`dismiss`/`skip`/`goto` methods to compose your own behavior.
...
Both `dismiss` and `skip` end the tour and trigger `onStatusChange`, but with a
different `status` (`"dismissed"` vs `"skipped"`) so you can distinguish user
intent, for example, to decide whether to show the tour again later.
...
After the tour closes, `api.step` still references the last step so exit
animations can keep rendering content. Drive visibility with `api.open` (or
Presence), and read content from `api.step`.

```tsx
{
  api.step && (
    <Presence present={api.open} unmountOnExit>
...
> Don't treat `api.step === null` as "tour closed". Use `api.open` or
> `onStatusChange` for that.
...
Each part includes a component-scoped data attribute that can be used to style
it in the DOM.
...
[data-tour-progress-text] {
  /* styles for the progress text part */
}

[data-tour-action-trigger] {
...
```css
[data-tour-content][data-type="dialog"] {
  /* styles for content when step is dialog type */
}

[data-tour-content][data-type="floating"] {
  /* styles for content when step is floating type */
}

[data-tour-content][data-type="tooltip"] {
  /* styles for content when step is tooltip type */
}

[data-tour-positioner][data-type="dialog"] {
  /* styles for positioner when step is dialog type */
}

[data-tour-positioner][data-type="floating"] {
...
The machine sets `--tour-layer` on the layered parts (`0` backdrop, `1`
spotlight, `2` positioner). Apply it in CSS so content stacks above the
backdrop:
...
The spotlight exposes CSS variables you can transition between steps:
...
Variables: `--spotlight-x`, `--spotlight-y`, `--spotlight-width`,
`--spotlight-height`.
...
```css
[data-tour-positioner][data-type="floating"][data-placement*="bottom"] {
  /* styles for bottom placement */
}

[data-tour-positioner][data-type="floating"][data-placement*="top"] {
  /* styles for top placement */
}

[data-tour-positioner][data-type="floating"][data-placement*="start"] {
  /* styles for start placement */
}

[data-tour-positioner][data-type="floating"][data-placement*="end"] {
