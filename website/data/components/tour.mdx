...
title: Tour
description:
  Used to onboard users with a guided, step-by-step walkthrough of your product.
package: "@zag-js/tour"
---
...
### Resolving a step's target

`target` is called on every position update, so a step keeps up when the page
swaps its element. Keep it cheap and return a stable reference, since it runs on
scroll and resize. Returning `null` keeps the last resolved element.
