...
When customizing element IDs, always use the `ids` option in the machine
context. Never manually set the `id` attribute on elements using the prop
functions.
...
return (
  <label {...api.getLabelProps()} id="my-custom-id">
    Label
  </label>
)
